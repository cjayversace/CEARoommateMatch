...
def load(file):
    with open(file, newline='') as csvfile:
        ...
        for row in reader:
            ...
            Stud.append([row['Name'], row['Classification?'].lower(), row['Major?'].lower(), row['Do you like having guests?'].lower(), row['Introvert/Extrovert?'].lower(), row['Light / Heavy sleeper '].lower(), row['Morning / Night person?'].lower(), row['Clean / Messy?'].lower(), row['Down to Party?'].lower(), row['Can you cook?'].lower(), row['Gym buddies? '].lower(), row['Weekend spent? '].lower(), row['Fav Music'].lower()])
...
def majcmp(subj, obj):
    #abbreviations
    maj = [['cs', 'computer', 'science'], ['arch', 'architecture', 'arc'], [ 'engineering', 'ae', 'me', 'aero/astro', 'aero', 'mechanical', 'meche', 'industrial', 'Chem', 'chemical',]]
